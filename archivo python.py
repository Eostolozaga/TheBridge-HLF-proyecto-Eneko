...
class Board:
    def __init__(self, owner, es_tablero_disparos=False):
        ...
        self.coordenadas_ocupadas = set()# Conjunto (set) para verificar rápidamente si una casilla ya tiene un barco
    ...
    def colocar_barcos_aleatorio(self, lista_barcos):
        ...
        for barco in lista_barcos:
            colocado = False
            while not colocado:
                orientacion = random.choice(['H', 'V']) # En random.choice se puede pasar una lista de opciones y devuelve una al azar
...
class Partida:
    ...
    def crear_jugadores_y_tableros(self):
        ...
        self.humano.tablero_propio = Board("Humano", False) # El False determina que este tablero se usará para colocar barcos, no para dispararlos y "Humano" es el dueño.
    ...
    def crear_barcos_iniciales(self):
        ...
        barcos = []
        barcos.extend([Barco(1) for _ in range(4)]) #Al hacer el for in creamos una lista temporal de 4 barcos. Si usaramos append meteria la lista entera dentro de otra lista.
        barcos.extend([Barco(2) for _ in range(3)]) #Extend coge esos 4 barcos los saca de la lista temporal y los pone "sueltos" en la lista.
        barcos.extend([Barco(3) for _ in range(2)])
        barcos.append(Barco(4)) # Al ser solo uno se puede usar el append
        return barcos
    
    def flota_hundida(self, jugador):
        # Verifica si todos los barcos del jugador han sido destruidos
        return all(not barco.esta_vivo() for barco in jugador.barcos) # El all devuelve True si todos los barcos están hundidos (no vivos)
    ...
    def obtener_barco_en_coordenada(self, jugador, x, y):
        ...
        for barco in jugador.barcos:
            if (x, y) in barco.coordenadas and barco.esta_vivo(): # Solo devuelve el barco si la coordenada coincide y el barco aún tiene vidas (no está hundido)
                return barco
        return None

    def mostrar_tableros_lado_a_lado(self): 
        # Renderiza visualmente ambos tableros del humano en la consola(genera la imagen)
    ...
    def celda_a_str(self, c):   
        ...
        if c == ' ': return ' ' # Lo convierte en string para evitar errores con el .join al intentar concatenar un espacio con un símbolo de barco o impacto.
        return str(c)
    ...
    def disparo_humano(self):
        ...
        partes = entrada.split(',') # El split separa la entrada en dos partes usando la coma como delimitador. Esto deja acceder a los dos elementos por separado.
        if len(partes) != 2:
        ...
        if self.trump.tablero_propio.celda[fila][col] == 'B':
            self.humano.tablero_disparos.celda[fila][col] = ROJO +'X'+ RESET # El reset es necesario para que no siga pintando de rojo.
            print("🎯 ¡TOCADO!")
    ...
    def jugar(self):
        ...
        while self.estado_general == "EN_CURSO":
            ...
            # Ejecuta el turno correspondiente.Los return False "apagan el interruptor" para salir del bucle si el juego ha terminado, de lo contrario, siguen alternando los turnos.
